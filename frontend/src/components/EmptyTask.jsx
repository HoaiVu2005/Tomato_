...
const EmptyTask = ({ filter }) => {
  return (
    <Card className="flex px-3 sm:px-none flex-col  justify-center text-center items-center shadow-2xl">
      <Circle className="size-13 text-slate-500" />
      {filter === "all" ? (
        <p className="font-medium text-sm sm:text-[18px]">Chưa có nhiệm vụ</p>
      ) : filter === "active" ? (
